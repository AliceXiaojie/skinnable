...
public class sketch2DPackingV03 extends PApplet {
        ...
        if(group.get(0).order == 0){ // ********** First Situation: if this group includes the first module 
          ...
          for(int k = 0; k <= groupWidth; k++){
             for(int l = 0; l <= maxHeight; l++){
               collision = false;
               int p = get(detectX + grid * k + 2, detectY + grid * l + 2); // avoid the grid lines and get color of the cell
               if(p != color(0,0,0)){
                 collision = true;
                 break;
               } 
...
class Module{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch2DPackingV03" };
    if (passedArgs != null) {
